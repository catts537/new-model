...
def get_line_direction(p1, p2):
    ...
    if 45 <= angle < 135: return "Top_to_Bottom"
    if 225 <= angle < 315: return "Bottom_to_Top"
    if 135 <= angle < 225: return "Right_to_Left"
    return "Left_to_Right"

def mouse_callback(event, x, y, flags, param):
    global drawing, current_line, lines, typing_name, mid_point, current_name
    if typing_name: return
    if event == cv2.EVENT_LBUTTONDOWN:
        if not drawing:
            ...
            mid_point = ((current_line[0][0]+current_line[1][0])//2, 
...
def check_line_crossing(p1, p2, line_p1, line_p2):
    # p1,p2 = previous & current position of object
